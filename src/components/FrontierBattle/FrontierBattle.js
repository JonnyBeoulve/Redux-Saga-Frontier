      ...
      {enemyAvatar && enemyData ? (
        <Fragment>
          <div className="frontier-enemy-menu">
            <div
              className={["enemy-character", css(styles.bounceIn)].join(" ")}
            >
              <img
                src={enemyAvatar}
                className="enemy-character-image"
                alt="Redux Saga Frontier enemy avatar"
              />
        ...
        <div className="score-menu">
          {score > 0 && (
            <h2 className="pulsate">You defeated {enemyData.name.last}!</h2>
          )}
          <h2>Current Score: {score}</h2>
          <div className="button-menu">
            <button className="primary-button" onClick={onCreateBattle}>
              Find Battle
            </button>
          </div>
        </div>
